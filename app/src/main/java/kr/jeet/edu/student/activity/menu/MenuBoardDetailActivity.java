package kr.jeet.edu.student.activity.menu;
...
//                        if (_pushData != null) if (_pushData.stCode == _stCode) {
...
//                    }
    ...
    private void requestSystemDetail(){
        if (RetrofitClient.getInstance() != null){
            ...
            if (RetrofitClient.getApiInterface() != null) {
                RetrofitClient.getApiInterface().getSystemNoticeDetail(_currentSeq).enqueue(new Callback<SystemNoticeResponse>() {
                    @Override
